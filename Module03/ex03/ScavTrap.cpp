...
void  ScavTrap::attack(const std::string& target) {
	if (this->_health > 0 && this->_energy > 0) {
		std::cout << "ScavTrap " << this->getName() << " attacks " << target << " causing " << _attackDamage << " points of damage!\n";
		_energy -= 1;
	} else {
		std::cout << "ScavTrap " << this->getName() << " can't attack cause of health or energy !\n";
	}
};
